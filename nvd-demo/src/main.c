...
int main(void) {
        if (nvd_init() != 0) die("Couldn't initialize NvDialog");

        NvdDialogBox *dialog = nvd_dialog_box_new(
                "Introduction",
                "This is a basic dialog box created with NvDialog.",
                NVD_DIALOG_SIMPLE);
        NvdDialogBox *warning = nvd_dialog_box_new(
                "Warning",
        ...
        NvdQuestionBox *question =
                nvd_dialog_question_new("Question",
                                        "Do you like NvDialog so far? (If not, "
        ...
        if (reply == NVD_REPLY_OK) {
                NvdDialogBox *success = nvd_dialog_box_new(
                        "Thanks",
                        "Thank you! Make sure to star the project on GitHub. "
                ...
                NvdDialogBox *error = nvd_dialog_box_new(
                        "Sad",
                        "That's alright, make sure to report any issues you "
        ...
        NvdNotification *notif = nvd_notification_new(
                "Whoop!", "Seems like a notification was sent from NvDialog...",
        ...
        NvdAboutDialog *about_dialog = nvd_about_dialog_new(
                ...
                "also used to test if the library works as expected across "
                "platforms.",
                NULL);
